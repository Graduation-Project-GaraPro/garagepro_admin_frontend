...
function StaffSectionImpl({
    ...
    <TabsContent value="available" className="space-y-4">
      {managersWithoutBranch.length > 0 && (
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {managersWithoutBranch.map((m) => (
              <StaffCheckbox key={m.id} staff={m} />
            ))}
          </div>
        </div>
      )}
      ...
      {techniciansWithoutBranch.length > 0 && (
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {techniciansWithoutBranch.map((t) => (
              <StaffCheckbox key={t.id} staff={t} />
            ))}
          </div>
        </div>
      )}
      ...
      {availableCount === 0 && (
        <div className="text-center py-8 text-muted-foreground">
          ...
          <div className="text-sm">
            All staff are currently assigned to branches
          </div>
        </div>
      )}
    </TabsContent>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <div className="font-medium text-orange-800">
            Staff Reassignment Warning
          </div>
          <div className="text-sm text-orange-700 mt-1">
            You have selected {Object.keys(warningMessages).length} staff member
            (s) who are already assigned to other branches. Saving this form
            will reassign them to this branch and remove them from their current
            assignments.
          </div>
        </div>
      </div>
  ...
  {errors.staffIds && (
    <p className="text-sm text-red-500">{errors.staffIds}</p>
  )}
</CardContent>
