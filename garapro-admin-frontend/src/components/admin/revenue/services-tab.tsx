    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <RechartsPieChart>
                <Pie
                  ...
                  labelLine={false}
                  label={(props) => {
                    const p = props as any; // tránh lỗi type ở chỗ này
                    const payload = p.payload ?? {};
                    const serviceName =
                      p.name ?? payload.serviceName ?? "Unknown";
                    // nếu lib trả percent (0..1) thì nhân 100; nếu bạn lưu trực tiếp percentageOfTotal trong payload, fallback vào đó
                    const percentage =
                      typeof p.percent === "number"
                        ? Math.round((p.percent ?? 0) * 100)
                        : payload.percentageOfTotal ?? 0;
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Number of orders for each service type
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Detailed breakdown of top performing services
          </CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">
                      {service.orderCount} orders • Avg:{" "}
                      {formatCurrency(service.revenue / service.orderCount)}
                    </div>
                  </div>
                </div>
