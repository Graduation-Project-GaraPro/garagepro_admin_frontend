...
class UserService {
  ...
  async getUsers(filters?: UserFilters): Promise<UserResponse> {
    try {
      ...
      const res = { users: pageItems, total, page, limit, totalPages };

      console.log("Mock data response:", res);
      return res;
    }
  }
  ...
  async banUser(id: number, reason: string): Promise<void> {
    try {
      await apiClient.put(`${this.baseUrl}/${id}/ban`, { message: reason });
    } catch (error) {
  ...
  async unbanUser(id: number): Promise<void> {
    try {
      await apiClient.put(`${this.baseUrl}/${id}/unban`);
    } catch (error) {
  ...
  async exportUser(
    ...
    try {
      ...
      // Fallback export
      if (format === "json") {
        return new Blob([JSON.stringify(user, null, 2)], {
  ...
  async bulkDeleteUsers(userIds: number[]): Promise<void> {
    try {
      await apiClient.delete(`${this.baseUrl}/bulk-delete`, {
        body: { userIds },
      });
    } catch (error) {
  ...
  async sendNotification(
    ...
    try {
      await apiClient.post(`${this.baseUrl}/${userId}/notifications`, {
        message,
        type,
      });
    } catch (error) {
