...
class StatisticsService {
  ...
  async getOverviewAnalytics(
    ...
    try {
      ...
      console.warn(
        "Failed to fetch overview analytics from API, using fallback data:",
        error
      );
  ...
  async getMetrics(filters?: StatisticsFilters): Promise<MetricData[]> {
    ...
    try {
      ...
      console.warn(
        "Failed to fetch metrics from API, using fallback data:",
        error
      );
  ...
  async getGeographicData(): Promise<GeographicData[]> {
    ...
    try {
      const response = await apiClient.get<GeographicData[]>(
        `${this.baseUrl}/realtime/geographic`
      );
  ...
  async generateReport(
    ...
    try {
      ...
      console.warn(
        "Report generation API not available, creating fallback:",
        error
      );
  ...
  async getCustomAnalytics(
    ...
    try {
      const response = await apiClient.post<any>(`${this.baseUrl}/custom`, {
        query,
        filters,
      });
      return response.data;
      ...
      console.warn(
        "Custom analytics not available, providing basic response:",
        error
      );
      return {
        query,
  ...
  async getRealTimeMetrics(): Promise<RealTimeMetric[]> {
    ...
    try {
      const response = await apiClient.get<RealTimeMetric[]>(
  ...
  async getLiveActivities(): Promise<LiveActivity[]> {
    try {
      const response = await apiClient.get<LiveActivity[]>(
        `${this.baseUrl}/realtime/activities`
      );
      return response.data;
    } catch (error) {
  ...
  private generateFallbackExport(
    type: string,
    filters?: StatisticsFilters
  ): Blob {
    let csvContent = "";
    ...
    switch (type) {
      ...
      case "garage-branches":
        ...
        const garageBranches = this.getFallbackMetrics().find(
          (m) => m.id === "garage-branches"
        );
        if (garageBranches && garageBranches.branches) {
          garageBranches.branches.forEach((branch) => {
            csvContent += `"${branch.name}","${branch.city}","${
              branch.state
            }","${branch.managerName}",${branch.staff},"${
